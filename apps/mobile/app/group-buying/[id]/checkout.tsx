/**
 * 공동구매 결제 — 광장 web /group-buying/[id]/checkout 1:1 미러 (RN 버전).
 *
 * 정독 매핑:
 *   - 헤더 (← 결제하기)
 *   - 상품 카드 (썸네일 + 제목 + 단가)
 *   - 수량 stepper
 *   - 받는 방법 (pickup / delivery)
 *   - 픽업: post.pickup_location / pickup_time 표시
 *   - 배송: 받는 사람·연락처·주소 입력
 *   - 메모 프리셋
 *   - 포인트 사용 (잔액 + 한도 max%)
 *   - 결제 합계 (수량×단가 - 포인트)
 *   - 결제 버튼 (createGbOrder + payGbOrder mock)
 *
 * 우편번호 검색은 RN 미지원 — 사용자가 직접 입력.
 * 본인 결제 진행 후 router.replace('/(tabs)/mypage') (web 은 /mypage/orders).
 */
...
export default function GroupBuyingCheckoutScreen() {
  ...
  const [receiveMethod, setReceiveMethod] = useState<"pickup" | "delivery">("delivery")
  ...
  useEffect(() => {
    ...
    getPost(supabase, id, DEFAULT_PLAZA).then(({ post }) => {
      if (!post) {
      ...
      if (post.user_id === user.id) {
        Alert.alert("본인 글에는 참여할 수 없습니다")
        router.back()
        return
      }
      setPost(post)
      if (post.status !== "recruiting") {
        Alert.alert("모집이 마감된 공구입니다")
      }
      // 🅲 profile 자동 채움 (이름, 전화) — 현재 광장 plaza_profiles 우선 (phone)
      ;(async () => {
    ...
    gwangjangFetch("/api/points/balance")
      ...
      .catch(() => {})
    // 포인트 사용 정책 — 관리자 설정값 반영 (하드코딩 30% 대신)
    supabase
  ...
  const shippingFee = isFreeShipping ? 0 : ((post as any)?.delivery_fee as number) || 0
  ...
  async function handleSubmit() {
    ...
    // 모집 마감된 공구는 결제 차단 — 마감 상태로 결제 시도 방지
    if (post.status !== "recruiting") {
      Alert.alert("참여 불가", "이미 모집이 마감된 공구입니다.")
      return
    }
    ...
    setSubmitting(true)
    try {
      const r = await createGbOrder(
        (u, init) => gwangjangFetch(u, init as any),
        {
          post_id: id,
          quantity,
          receive_method: receiveMethod,
          delivery_addr: receiveMethod === "delivery"
            ? {
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.body}>
          ...
          <View style={styles.productCard}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.productName} numberOfLines={2}>{post.product_name}</Text>
              <Text style={styles.productPrice}>{post.group_price.toLocaleString()}원</Text>
            </View>
          </View>
            ...
            <Field label="받는 방법">
              <View style={{ flexDirection: "row", gap: spacing[2] }}>
                {(["delivery", "pickup"] as const).map((m) => (
                  <Pressable
                    key={m}
                    onPress={() => setReceiveMethod(m)}
                    style={[
                      styles.chipBtn,
                      receiveMethod === m && styles.chipBtnActive,
                    ]}
                  >
                    <Text
                      style={[
                        styles.chipBtnText,
                        receiveMethod === m && styles.chipBtnTextActive,
                      ]}
                    >
                      {m === "delivery" ? "배송" : "직접 수령"}
                    </Text>
          ...
          {receiveMethod === "pickup" && post.pickup_location ? (
            <View style={styles.pickupBox}>
              <View style={styles.pickupRow}>
                <Ionicons name="location-outline" size={16} color={lightColors.ink700} />
                <Text style={styles.pickupText}>{post.pickup_location}</Text>
              </View>
              {(post as any).pickup_time ? (
                <View style={styles.pickupRow}>
                  <Ionicons name="time-outline" size={16} color={lightColors.ink700} />
                  <Text style={styles.pickupText}>{(post as any).pickup_time}</Text>
                </View>
              ) : null}
            </View>
          ) : null}
            ...
            <View style={styles.chipWrap}>
              {MEMO_PRESETS.map((p) => (
                <Pressable
                  ...
                  style={[styles.chip, memoPreset === p ? { backgroundColor: PURPLE } : { backgroundColor: lightColors.muted }]}
                >
          ...
          <Field label={`포인트 사용 (잔액 ${availablePoints.toLocaleString()}P)`}>
            <TextInput
              value={pointsInput}
              onChangeText={(v) => {
                // 입력값을 사용 가능 최대치로 클램프 — 표시값과 실제 적용값 불일치 방지
          ...
          <CheckoutConsentSection
            sellerName={(post as any)?.seller_nickname || (post as any)?.profiles?.nickname}
            onChange={setConsented}
          />

          <Pressable
            onPress={handleSubmit}
            disabled={submitting || !consented || post.status !== "recruiting"}
            style={[styles.payBtn, (submitting || !consented || post.status !== "recruiting") && { opacity: 0.5 }]}
          >
            {submitting ? (
              <ActivityIndicator color="#ffffff" />
            ) : post.status !== "recruiting" ? (
              <Text style={styles.payBtnText}>모집 마감된 공구</Text>
            ) : (
              <>
...
const styles = StyleSheet.create({
  ...
  productPrice: { fontSize: fontSize.md, fontWeight: "700", color: PURPLE },
  ...
  chipBtn: {
    ...
    borderWidth: 1, borderColor: lightColors.ink200, alignItems: "center" as const,
  },
  chipBtnActive: { borderColor: PURPLE, backgroundColor: "rgba(139,92,246,0.08)" },
  chipBtnText: { fontSize: fontSize.sm, color: lightColors.ink600, fontWeight: "500" as const },
  chipBtnTextActive: { color: PURPLE },
  ...
  pickupText: { fontSize: fontSize.sm, color: lightColors.ink900 },
  ...
  totalValMain: { fontSize: fontSize.lg, fontWeight: "700", color: PURPLE },

  payBtn: {
    flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8,
    paddingVertical: 14, borderRadius: radius.md, backgroundColor: PURPLE,
  },
