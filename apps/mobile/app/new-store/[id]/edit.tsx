/**
 * 신장개업 수정 — 광장 web /new-store/[id]/edit 미러.
 * register form 동일 + prefill + PATCH /api/new-store/[id].
 */
...
export default function NewStoreEditScreen() {
  ...
  useEffect(() => {
    if (!id) return
    getNewStorePost(getSupabase(), id, DEFAULT_PLAZA, null).then(({ post }) => {
      if (post) {
        ...
        setPhone(post.phone || "")
        setOpeningDate(post.opening_date || "")
        setOpeningEvent(post.opening_event || "")
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true)
    try {
      const r = await updateNewStorePost(
      ...
      await setPostRegion("new_store_posts", id, regionId)
      Alert.alert("수정 완료", "신장개업 글이 수정되었습니다")
      setFormDirty(false)
      router.replace(`/new-store/${id}` as any)
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>신장개업 수정</Text>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.body}>
          ...
          <Field label="매장명 *">
            <TextInput value={storeName} onChangeText={setStoreName} style={styles.input} placeholderTextColor={lightColors.ink500} />
          </Field>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6 }}>
              {categories.map((c) => (
                <Pressable
                  ...
                  style={[
                    styles.chip,
                    category === c ? { backgroundColor: STORE_BLUE } : { backgroundColor: lightColors.muted },
                  ]}
                >
          ...
          <Field label="전화번호">
            <TextInput value={phone} onChangeText={setPhone} keyboardType="phone-pad" style={styles.input} placeholderTextColor={lightColors.ink500} />
          </Field>

          <Field label="오픈일">
            <DatePickerField value={openingDate} onChange={setOpeningDate} mode="date" placeholder="날짜 선택" clearable />
          </Field>

          <Field label="오픈 이벤트">
            <TextInput value={openingEvent} onChangeText={setOpeningEvent} placeholder="예: 오픈 기념 전 메뉴 20% 할인!" style={styles.input} placeholderTextColor={lightColors.ink500} />
          </Field>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  saveBtn: {
    ...
    backgroundColor: STORE_BLUE, minWidth: 60, alignItems: "center",
  },
