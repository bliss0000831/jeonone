/**
 * 서비스 수정 공용 컴포넌트 — moving / cleaning / repair edit thin wrapper 가 사용.
...
export function ServiceEdit({ kind, id }: { kind: ServiceKind; id: string }) {
  ...
  useEffect(() => {
    if (!id) return
    getServicePost(getSupabase(), kind, id, DEFAULT_PLAZA, null).then(({ post }) => {
      if (post) {
        setTitle(post.title || "")
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true)
    try {
      const r = await updateServicePost(getSupabase(), kind, id, {
        plaza: DEFAULT_PLAZA,
        userId: "",
        title: title.trim(),
      ...
      Alert.alert("수정 완료", `${meta.label} 글이 수정되었습니다`)
      setFormDirty(false)
      router.replace(`/${kind}/${id}` as any)
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{meta.label} 수정</Text>
        <Pressable
          onPress={handleSubmit}
          disabled={submitting || uploading}
          style={[styles.saveBtn, { backgroundColor: meta.bg }, submitting && { opacity: 0.5 }]}
        >
            ...
            <View style={styles.chipWrap}>
              {categories.map((c) => (
                <Pressable
                  ...
                  style={[styles.chip, category === c ? { backgroundColor: meta.bg } : { backgroundColor: lightColors.muted }]}
                >
...
const styles = StyleSheet.create({
  ...
  saveBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: radius.md, minWidth: 60, alignItems: "center" },
