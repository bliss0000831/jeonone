...
export async function GET(
  ...
  let q: any = (supabase as any).from("service_requests").select("*").eq("id", id)
  ...
  void supabase.rpc('increment_view_count', { p_table: 'service_requests', p_id: id, p_column: 'views' })
  ...
  const { data: responses } = await (supabase as any)
    ...
    .select("id, user_id, request_id, content, created_at")
    .eq("request_id", id)
    .order("created_at", { ascending: true })

  const responderIds = [...new Set((responses ?? []).map((r: any) => r.user_id))] as string[]
  ...
  const responsesWithAuthor = (responses ?? []).map((r: any) => ({
    ...r,
...
export async function PATCH(
  ...
  const keys = ["title", "content", "service_type", "region", "district", "dong", "budget_min", "budget_max", "desired_date", "sub_region"] as const
  for (const k of keys) {
  ...
  await safeRevalidate(`/service-requests/${id}`)
