...
import { HandHeart, ArrowLeft, MapPin, CalendarDays, Loader2, Trash2, MessageSquare, Send, Briefcase, CheckCircle2, MoreVertical, Pencil } from "lucide-react"
...
export default function RequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!user) {
      router.push(`/auth/login?next=/requests/${id}`)
      return
    }
    if (!responseText.trim()) return
    setSubmitting(true)
    try {
      const res = await fetch(`/api/property-requests/${id}/responses`, {
        method: "POST",
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    const res = await fetch(`/api/property-requests/${id}`, { method: "DELETE" })
    ...
    return (
      <div className="min-h-screen bg-background pb-20 md:pb-0">
        ...
        <main className="max-w-2xl mx-auto px-4 py-12 text-center">
          ...
          <Link href="/requests" className="inline-block mt-4 text-sm text-rose-600">목록으로</Link>
        </main>
        <BottomNav />
      </div>
    )
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
          ...
          <div className="flex items-start justify-between gap-2 mb-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {req.transaction_type && (
                <span className="text-[11px] px-2 py-0.5 rounded-full bg-rose-50 text-rose-600 dark:bg-rose-950/40 dark:text-rose-300 font-semibold">
                  {req.transaction_type}
                </span>
              )}
              {req.property_type && (
              ...
              {(isOwner || isAdmin) && (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => router.push(`/requests/${id}/edit`)}>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs mb-4">
            ...
            {req.move_in_date && (
              <div className="flex items-center gap-1.5 text-muted-foreground">
                ...
                <span>{new Date(req.move_in_date).toLocaleDateString("ko-KR")}</span>
              </div>
            )}
          </div>
        ...
        <div className="mb-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-bold">
              응답 <span className="text-rose-600">{responses.length}</span>
            </h2>
          </div>
          ...
          {responses.length === 0 ? (
            <div className="rounded-xl border border-dashed border-border bg-muted/30 p-6 text-center">
              <p className="text-xs text-muted-foreground">
                아직 응답이 없어요. {accountType === "agent" ? "첫 번째로 매물을 추천해보세요!" : "공인중개사의 응답을 기다려주세요"}
              </p>
            </div>
          ) : (
                ...
                return (
                  <div
                    ...
                    className={`rounded-xl border p-3 ${
                      isAgent
                        ? "border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 dark:border-blue-900/40"
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        {isAgent && (
                          <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-blue-500 text-white text-[9px] font-bold">
        ...
        {req.status === "open" && (
          <form onSubmit={handleSubmit} className="rounded-2xl border border-border bg-card p-3">
            {!user ? (
              <Link
                href={`/auth/login?next=/requests/${id}`}
                className="block text-center py-3 text-sm text-rose-600 font-medium"
              >
                로그인하고 응답하기
              </Link>
            ) : (
              <>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-rose-500/30 resize-none"
                />
                <div className="flex justify-end mt-2">
                  <button
                    type="submit"
                    disabled={submitting || !responseText.trim()}
                    className="flex items-center gap-1 px-4 py-2 bg-rose-500 text-white rounded-full text-sm font-medium hover:bg-rose-600 transition-colors disabled:opacity-50"
                  >
