...
export default function MovingDetailPage() {
  ...
  const [post, setPost] = useState<MovingPost | null>(null)
  ...
  const { handleChat, chatLoading } = usePostChat({
    ...
    loginRedirectPath: typeof id === "string" ? `/moving/${id}` : undefined,
  })
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      let q: any = supabase.from("moving_posts").select("*").eq("id", id)
      ...
      if (data) {
        const { data: profileData } = await supabase
          ...
          .eq("id", data.user_id)
          .single()
        setPost({ ...data, profiles: profileData } as MovingPost)

        // 조회수 +1 — atomic RPC (race-free)
        void supabase.rpc('increment_view_count', { p_table: 'moving_posts', p_id: id, p_column: 'views' })
  ...
  const handleDelete = async () => {
    ...
    const supabase = createClient()
    const { error } = await supabase.from("moving_posts").delete().eq("id", id)
    if (error) {
      ...
      if (next) {
        ...
        if (error) {
          ...
          void supabase.rpc("change_like_count", {
            p_table: "moving_posts", p_id: id, p_column: "likes", p_delta: 1,
          }).then(({ error: rpcErr }) => { if (rpcErr) console.error("[moving like]", rpcErr) })
        }
      } else {
        const { error } = await supabase
          .from("moving_favorites")
        ...
        void supabase.rpc("change_like_count", {
          p_table: "moving_posts", p_id: id, p_column: "likes", p_delta: -1,
        }).then(({ error: rpcErr }) => { if (rpcErr) console.error("[moving unlike]", rpcErr) })
      }
    } catch (e) {
      console.error("[moving like]", e)
  ...
  return (
          ...
          extra={
            <>
              {!(currentUser && currentUser.id === post.user_id) && !isAdmin && (
                <ReportButton targetType="moving" targetId={post.id} variant="icon" />
              )}
              <BumpQuickMenu
    ...
    >
      <DetailGallery
        ...
        alt={post.title}
        fallbackIcon={Truck}
        fallbackLabel={post.category || "이사"}
        topLeftBadges={
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium text-white bg-yellow-500 shadow-sm">
      ...
      <DetailBody>
        ...
        <DetailSection title="업체 정보">
          <DetailAuthorCard
            ...
            name={post.profiles?.nickname || "이사 전문가"}
            avatarUrl={post.profiles?.avatar_url}
            badges={
              <span className="inline-flex items-center gap-1 text-xs font-medium text-yellow-700 bg-yellow-50 dark:bg-yellow-950/40 dark:text-yellow-400 px-2 py-0.5 rounded-full">
                <Truck className="w-3 h-3" />
                이사
              </span>
            }
            userId={post.user_id}
            otherPostsTable="moving_posts"
            otherPostsLinkPrefix="/moving"
