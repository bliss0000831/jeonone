...
const CATEGORY_OPTIONS = [
  { value: "전체", label: "전체" },
  { value: "가정이사", label: "가정이사" },
  { value: "원룸이사", label: "원룸이사" },
  { value: "사무실이사", label: "사무실이사" },
  { value: "용달이사", label: "용달이사" },
  { value: "포장이사", label: "포장이사" },
]

const PRICE_OPTIONS = [
  { value: "all", label: "전체" },
  { value: "0-300000", label: "~30만원" },
  { value: "300000-600000", label: "30~60만원" },
  { value: "600000-1000000", label: "60~100만원" },
...
function MovingListPageContent() {
  ...
  useEffect(() => {
    ...
    router.replace(qs ? `/moving?${qs}` : "/moving", { scroll: false })
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        <PageHero
          pageKey="moving"
          bannerImage="/banners/moving-banner.jpg"
          eyebrow="홈즈 · 짐 싸기 전에"
          icon={<Truck className="w-7 h-7 sm:w-8 sm:h-8 text-amber-300" />}
          title="우리 동네"
          titleAccent="이사"
          accentGradient="from-amber-300 to-yellow-300"
          subtitle="믿을 수 있는 동네 이사 업체를 한눈에"
          action={
            user && (
              <Link
                href="/moving/register"
                className="flex items-center gap-1 px-4 py-2 bg-yellow-500 text-white rounded-full text-sm font-medium hover:bg-yellow-600 transition-colors shadow-lg shadow-black/20"
              >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {filtered.map((post) => (
              <ServiceCard key={post.id} post={post} serviceType="moving" currentUserId={user?.id} />
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-lg font-medium text-foreground mb-2">
              {posts.length === 0 ? "아직 등록된 이사 서비스가 없어요" : "검색 결과가 없어요"}
            </h3>
            <p className="text-muted-foreground text-sm">
              {posts.length === 0 ? "첫 번째 이사 서비스를 등록해보세요!" : "다른 조건으로 검색해보세요"}
            </p>
          </div>
        )}
      </main>
...
export default function MovingListPageClient() {
  return (
    <Suspense
      fallback={
