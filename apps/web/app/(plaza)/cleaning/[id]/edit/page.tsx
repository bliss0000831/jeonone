...
import { ChevronLeft, Plus, X, Home, Building, Sparkles, Trash2, MoreHorizontal, Loader2 } from "lucide-react"
...
const categories = [
  { value: "입주청소", label: "입주청소", icon: Home },
  { value: "이사청소", label: "이사청소", icon: Building },
  { value: "정기청소", label: "정기청소", icon: Sparkles },
  { value: "특수청소", label: "특수청소", icon: Trash2 },
...
export default function EditCleaningPage({ params }: EditCleaningPageProps) {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      if (!user) {
        router.push(`/auth/login?redirect=/cleaning/${id}/edit`)
        return
      }
    ...
    <div className="min-h-screen bg-background">
      <header className="safe-top sticky top-0 z-50 bg-background border-b border-border">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <h1 className="font-semibold">청소 서비스 수정</h1>
          <div className="w-10" />
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-6 max-w-2xl mx-auto pb-24">
        ...
        <div>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {images.length < 10 && (
              <label className="w-20 h-20 border-2 border-dashed border-border rounded-lg flex items-center justify-center cursor-pointer hover:border-pink-500 transition-colors">
