...
import {ChevronLeft, Plus, SprayCan, Home, Building, Sparkles, Wind, MoreHorizontal} from "lucide-react"
...
const categories = [
  { value: "입주청소", label: "입주청소", icon: Home, description: "입주 전후 청소" },
  { value: "이사청소", label: "이사청소", icon: SprayCan, description: "이사 후 청소" },
  { value: "사무실청소", label: "사무실청소", icon: Building, description: "사무실, 상업시설 청소" },
  { value: "정기청소", label: "정기청소", icon: Sparkles, description: "정기 방문 청소" },
  { value: "에어컨청소", label: "에어컨청소", icon: Wind, description: "에어컨 분해 세척" },
  { value: "기타", label: "기타", icon: MoreHorizontal, description: "기타 청소 서비스" },
]

export default function CleaningRegisterPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!isAdmin && profile?.account_type !== "cleaning") {
        toast("청소 전문가 권한이 필요합니다")
        router.push("/")
        return
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 광장 가드 — setLoading(true) 호출 *전에* 검증해야 버튼 영구 비활성화 방지
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="safe-top sticky top-0 z-50 bg-background border-b border-border">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <h1 className="font-semibold">청소 서비스 등록</h1>
          <div className="w-10" />
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} onChange={() => setFormDirty(true)} className="p-4 space-y-6 max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-2">
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  <Icon className={cn("w-6 h-6", formData.category === cat.value ? "text-pink-500" : "text-muted-foreground")} />
                  <span className={cn("text-sm font-medium", formData.category === cat.value ? "text-pink-600" : "text-foreground")}>
                    {cat.label}
                  </span>
                </button>
              )
        ...
        {/* Submit */}
        <Button type="submit" className="w-full bg-pink-500 hover:bg-pink-600" disabled={loading || !consented}>
          {loading ? "등록 중..." : "등록하기"}
        </Button>
      </form>
    </div>
