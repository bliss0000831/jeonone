...
import {ChevronLeft, Plus, Wrench, Zap, Droplets, Tv, Paintbrush, MoreHorizontal} from "lucide-react"
...
const categories = [
  { value: "전기수리", label: "전기수리", icon: Zap, description: "전기, 조명 수리" },
  { value: "배관수리", label: "배관수리", icon: Droplets, description: "배관, 누수 수리" },
  { value: "도배장판", label: "도배장판", icon: Paintbrush, description: "도배, 장판 시공" },
  { value: "가전수리", label: "가전수리", icon: Tv, description: "가전 제품 수리" },
  { value: "잡수리", label: "잡수리", icon: Wrench, description: "각종 소소한 수리" },
  { value: "기타", label: "기타", icon: MoreHorizontal, description: "기타 수리 서비스" },
]
...
export default function RepairRegisterPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!isAdmin && profile?.account_type !== "repair") {
        toast("수리 전문가 권한이 필요합니다")
        router.push("/")
        return
      }
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="safe-top sticky top-0 z-50 bg-background border-b border-border">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <h1 className="font-semibold">수리 서비스 등록</h1>
          <div className="w-10" />
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} onChange={() => setFormDirty(true)} className="p-4 space-y-6 max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-2">
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  <Icon className={cn("w-6 h-6", formData.category === cat.value ? "text-orange-500" : "text-muted-foreground")} />
                  <span className={cn("text-sm font-medium", formData.category === cat.value ? "text-orange-600" : "text-foreground")}>
                    {cat.label}
                  </span>
                </button>
              )
        ...
        {/* Submit */}
        <Button type="submit" className="w-full bg-orange-500 hover:bg-orange-600" disabled={loading || !consented}>
          {loading ? "등록 중..." : "등록하기"}
        </Button>
      </form>
    </div>
