...
export default function RepairDetailPage() {
  ...
  const [post, setPost] = useState<RepairPost | null>(null)
  ...
  const { handleChat, chatLoading } = usePostChat({
    ...
    loginRedirectPath: typeof id === "string" ? `/repair/${id}` : undefined,
  })
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      let q: any = supabase.from("repair_posts").select("*").eq("id", id)
      ...
      if (data) {
        ...
        setPost({ ...data, profiles: profileData } as RepairPost)

        // 조회수 +1 — atomic RPC (race-free)
        void supabase.rpc('increment_view_count', { p_table: 'repair_posts', p_id: id, p_column: 'views' })
      ...
      if (next) {
        ...
        if (error) {
          ...
          void supabase.rpc("change_like_count", {
            p_table: "repair_posts", p_id: id, p_column: "likes", p_delta: 1,
          }).then(({ error: rpcErr }) => { if (rpcErr) console.error("[repair like]", rpcErr) })
        }
      } else {
        const { error } = await supabase
          .from("repair_favorites")
        ...
        void supabase.rpc("change_like_count", {
          p_table: "repair_posts", p_id: id, p_column: "likes", p_delta: -1,
        }).then(({ error: rpcErr }) => { if (rpcErr) console.error("[repair unlike]", rpcErr) })
      }
    } catch (e) {
      console.error("[repair like]", e)
  ...
  return (
          ...
          extra={
            <>
              {!(currentUser && currentUser.id === post.user_id) && !isAdmin && (
                <ReportButton targetType="repair" targetId={post.id} variant="icon" />
              )}
              <BumpQuickMenu
    ...
    >
      <DetailGallery
        ...
        alt={post.title}
        fallbackIcon={Wrench}
        fallbackLabel={post.category || "수리"}
        topLeftBadges={
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium text-white bg-orange-500 shadow-sm">
      ...
      <DetailBody>
        ...
        <DetailSection title="업체 정보">
          <DetailAuthorCard
            ...
            name={post.profiles?.nickname || "수리 전문가"}
            avatarUrl={post.profiles?.avatar_url}
            badges={
              <span className="inline-flex items-center gap-1 text-xs font-medium text-orange-600 bg-orange-50 dark:bg-orange-950/40 dark:text-orange-400 px-2 py-0.5 rounded-full">
                <Wrench className="w-3 h-3" />
                수리
              </span>
            }
            userId={post.user_id}
            otherPostsTable="repair_posts"
            otherPostsLinkPrefix="/repair"
