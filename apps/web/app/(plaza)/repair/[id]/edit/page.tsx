...
import { ChevronLeft, Plus, X, Wrench, Zap, Droplet, Wind, MoreHorizontal, Loader2 } from "lucide-react"
...
const categories = [
  { value: "종합수리", label: "종합수리", icon: Wrench },
  { value: "전기", label: "전기", icon: Zap },
  { value: "배관", label: "배관", icon: Droplet },
  { value: "에어컨", label: "에어컨", icon: Wind },
...
export default function EditRepairPage({ params }: EditRepairPageProps) {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      if (!user) {
        router.push(`/auth/login?redirect=/repair/${id}/edit`)
        return
      }
    ...
    <div className="min-h-screen bg-background">
      <header className="safe-top sticky top-0 z-50 bg-background border-b border-border">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <h1 className="font-semibold">수리 서비스 수정</h1>
          <div className="w-10" />
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-6 max-w-2xl mx-auto pb-24">
        ...
        <div>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {images.length < 10 && (
              <label className="w-20 h-20 border-2 border-dashed border-border rounded-lg flex items-center justify-center cursor-pointer hover:border-orange-500 transition-colors">
