...
// 목록 필터(/interior)의 시공종류 옵션과 동일하게 유지 (전체 제외)
const categories = [
...
export default function InteriorRegisterPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      const { data: profile } = await supabase
        .from("profiles")
        .select("account_type, phone, role")
        .eq("id", user.id)
        .single()
      ...
      if (profile) {
        ...
        if (!isAdmin && profile.account_type !== "interior") {
          toast("인테리어 전문가만 글을 등록할 수 있습니다.")
          router.push("/")
          return
        }
        if (profile.phone) {
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <header className="safe-top sticky top-0 z-40 bg-card border-b border-border">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <h1 className="font-semibold">인테리어 서비스 등록</h1>
          <div className="w-9" />
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} onChange={() => setFormDirty(true)} className="max-w-2xl mx-auto p-4 space-y-6">
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-card border-t border-border">
          <div className="max-w-2xl mx-auto">
            <Button type="submit" className="w-full bg-purple-500 hover:bg-purple-600 text-white" disabled={loading || !consented}>
